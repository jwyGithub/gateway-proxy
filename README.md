...
外网客户端 ──IPv6──> Windows 网关 (gateway-proxy)
                        │
                        ├─ https://relay.nicoo.eu.cc   ──> http://192.168.10.103:8080
                        ├─ https://web.nicoo.eu.cc     ──> http://192.168.10.103:3000
                        └─ develop.nicoo.eu.cc:2222    ──> 192.168.10.103:22 (SSH)
```
...
4. **SSH 路由**：L4 无域名概念，`develop.nicoo.eu.cc` 仅用于 DNS 解析，实际按 2222 端口路由
...
```
INFO 代理请求 method=GET host=relay.nicoo.eu.cc path=/ status=200 elapsed_ms=3
INFO 升级隧道建立 host=ws.nicoo.eu.cc
```
